...
export class TableComponent implements OnChanges, OnInit, DoCheck {
  ...
  constructor(private ngZone: NgZone,
    ...
    iconRegistry.addSvgIcon(
      'sort',
      sanitizer.bypassSecurityTrustResourceUrl('../assets/images/svg/sort.svg'));
      ...
      this.ngZone.run(() => {
        if (this.table.nativeElement.scrollWidth > this.table.nativeElement.clientWidth) {
          for (let i = this.internalHeaders.length - 1; i >= 0; i--) {
            if (this.internalHeaders[i].responsive === false) {
          ...
          this.internalHeaders.map(item => (item.responsiveSize && item.responsiveSize < this.table.nativeElement.clientWidth) ? item.responsive = false : '');
        }
      });
    };
  ...
  ngOnInit() {
    for (let i = 0; i < this.internalHeaders.length; i++) {
      if (i <= parseInt((this.table.nativeElement.scrollWidth / 130).toString())) {
        this.internalHeaders[i].responsive = false;
      } else {
        this.internalHeaders[i].responsive = true;
      }
    }
